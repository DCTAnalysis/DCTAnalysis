...
def map_categorical(image, label):
    """Maps labels to one-hot encoding, used for categorical hinge."""
    label = tf.one_hot([label], CLASSES)
    return image, label


def load_tfrecord(path, categorical=False):
    """Load tfrecords."""
    ...
    if categorical:
        # map labels into one-hot-encoding
        dataset.map(map_categorical)
...
def train(args):
    svm = args.MODEL == "svm"
    categorical = svm and CLASSES != 1
    train_dataset = load_tfrecord(args.TRAIN_DATASET, categorical=categorical)
    val_dataset = load_tfrecord(args.VAL_DATASET, categorical=categorical)
    ...
    with mirrored_strategy.scope():
        ...
        if CLASSES == 1:
            loss = tf.keras.losses.binary_crossentropy if not svm else tf.keras.losses.squared_hinge
        else:
            loss = tf.keras.losses.sparse_categorical_crossentropy if not svm else tf.keras.losses.categorical_hinge
        metrics = ["acc"]
...
def parse_args():
    ...
    train.add_argument("--early_stopping",
                       help=f"Early stopping criteria. Default: 2", type=int, default=2)
